...
type RankInput struct {
	...
	// Backtest overrides (zero = default live behavior).
...
func Rank(in RankInput) RankOutput {
	...
	if len(in.PlaybooksOnly) > 0 {
		sess.Playbooks = append([]string(nil), in.PlaybooksOnly...)
	}

	floor := AdaptiveFloor(sess, now, in.TradesToday, in.MinTradesPerDay)
	if in.FloorOverride > 0 {
		floor = in.FloorOverride
	}
	...
	for _, sym := range in.Symbols {
		...
		sig, flat := evalSymbol(sym, sess, floor, now, isChopRegime(sym.State), in)
...
func evalSymbol(sym SymbolInput, sess SessionProfile, floor int, now time.Time, chop bool, in RankInput) (model.ProSignal, bool) {
	...
	pbResults := []PlaybookResult{
		...
		evalVolClimaxFade(st),

		evalForcedFlowFade(st),
